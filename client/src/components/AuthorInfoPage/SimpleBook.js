...
const capitalizeTitle = (title) => {
  if (!title) return "Untitled"; // Handle null or undefined
  return title
    .split(" ")
...
export default function SimpleBook({ book }) {
    ...
    >
      <div style={styles.bookCoverContainer}>
        <img
          ...
          alt={`Book cover for ${book.title || "Untitled"}`}
          style={{
            ...styles.bookCover,
      ...
      <h2 style={styles.bookTitle}>{capitalizeTitle(book.title)}</h2>
      <p style={styles.bookGenre}>{capitalizeTitle(book.genres || "Unknown Genre")}</p>
      ...
      <p style={styles.authorName}>{book.publication_year || "Unknown Year"}</p>
    </article>
  );
}
